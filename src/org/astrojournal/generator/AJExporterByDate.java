...
public class AJExporterByDate {
   ...
   * @param latexReportsFolderByDate the directory containing the single observations by date in latex format (output)
   * @param latexHeaderByDate the latex header code (by date)
    ...
    for(int i=0; i<nObservations; i++) {
      ...
      try {
        table = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(
          new File(latexReportsByDateFolder, "obs" + filenameOut + ".tex")), "utf-8"));
