...
/*
 * Changelog:
 * - Piero Dalle Pezze: Code taken from the class HelpIndexRoot.java in 
...
public class HelpIndexRoot extends DefaultMutableTreeNode {
    ...
    private static final long serialVersionUID = -4876577148294194708L;
	...
	for (int f = 0; f < files.length; f++) {
	    ...
	    } else if (files[f].getName().toLowerCase().endsWith(".html")
		    || files[f].getName().toLowerCase().endsWith(".htm")) {
		HelpPage h = new HelpPage(files[f]);
		node.add(h);
	    }
